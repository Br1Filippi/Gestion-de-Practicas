...
<div class="col-9">
    <div class="container d-flex align-items-center justify-content-center">
        <div class="card custom-card shadow-sm" style="width: 70%; max-height: 90vh; overflow-y: auto;">
            <form method="POST" action="{{ route('secretarias.update', $secretaria->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-header bg-white">
                    ...
                    <div class="mb-3">
                        <input type="hidden" class="form-control @error('correo_usuario') is-invalid @enderror"
                            id="correo_usuario" name="correo_usuario" value="{{$secretaria->usuario->correo_usuario }}">
                    ...
                    <div class="mb-3">
                        <input type="hidden" class="form-control @error('password') is-invalid @enderror"
                            id="password" name="password" value="{{$secretaria->usuario->password}}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nombre') is-invalid @enderror" id="nombre"
                            name="nombre" value="{{ old('nombre', $secretaria->usuario->nombre) }}">
                        @error('nombre')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('apellido') is-invalid @enderror" id="apellido"
                            name="apellido" value="{{ old('apellido', $secretaria->usuario->apellido) }}">
                        @error('apellido')
                        <div class="text-danger d-flex" style="font-size: 0.8rem;">{{ $message }}</div>
                        @enderror
                    </div>
